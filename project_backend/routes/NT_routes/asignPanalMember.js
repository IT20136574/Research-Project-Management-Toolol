...
router.get("/fletchgroups/:status",(req,res)=>{
    ...
    if(status == "Accepted"){ 
        group.find({researchTopic_Status : status,panalmemberstatus : "Requested"}).exec((err,group)=>{
            if(err){
...
//fletch specific panel members
router.get("/fletchpanels/:field", (req,res)=>{
    let field = req.params.field;
    if(field){ 
         staff.find({field : field, role: "panal_member"}).exec((err,staff)=>{
            if(err){
                return res.status(400).json({
                    error:err
                });
...
router.post("/addPanelMembers/:id",async (req,res)=>{
    ...
    if(Group.panalmemberstatus === "Allocated"){
        throw new Error("Panel members already added")
    }   
    if(!Group){
